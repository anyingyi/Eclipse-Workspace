...
public class Simulations {
	...
	private static double[] runSimulation(String fileName, final int TIMES) {
		...
		for (int i = 0; i < Simulations.TIMES; i++) {
			...
			} else if (Simulations.methodType == EMethodType.PSO ) {
				s = Methods.basicPSO(); 
			}else {
	...
	public static String getParaSetting() {
		...
		if (methodType == EMethodType.PSO) {
			str += "ORDER";
			str += " popSize=" + popSize + " W=" + W_value +  "-" + Simulations.MAX_G;
		}
	...
	private static EMethodType methodType = EMethodType.NM;
	...
	//Parameter for PSO
	public static double W_value = 0.5;
}

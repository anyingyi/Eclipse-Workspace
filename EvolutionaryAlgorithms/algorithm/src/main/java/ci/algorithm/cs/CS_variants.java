...
import ci.Problem;
import ci.Solution;
import ci.problem.common.Griewank;
import util.Chaos;
...
public class CS_variants extends EvolutionaryAlgorithm {
    private double pa;
    private double beta;
    ...
    public CS_variants(Problem problem, long maxFES, int popSize, double pa, double beta){
    ...
    protected void LevyRandomWalk(double sigma, double beta){
        ...
        for (i = 0; i < this.populationSize; i++) {
            int rand=randGenerator.nextInt(populationSize);
            double[] distance=new double[dimensionSize];
            for(j=0;j<dimensionSize;j++){
                distance[j]=Math.abs(bestSolution.getVariableValue(j)-population.get(i).getVariableValue(j));
            }
            double averDistance= Statistic.average(distance);
            ...
            for (j = 0; j < this.dimensionSize; j++) {
                ...
                //bestx = population.get(rand).getVariableValue(j);
                bestx=bestSolution.getVariableValue(j);
                newx = currentx + alpha0 * step * this.randGenerator.nextGaussian() * (currentx - bestx);
                if(distance[j]>averDistance){
    ...
    protected void LocalRandomWalk() {
        ...
        for (int i = 0; i < this.populationSize; i++) {
            ...
            for(int j=0;j<dimensionSize;j++){
                double val=Math.abs(population.get(r1.get(i)).getVariableValue(j)-population.get(i).getVariableValue(j))
                            -Math.abs(population.get(r2.get(i)).getVariableValue(j)-population.get(i).getVariableValue(j));
                distance[j]=Math.abs(val);
            }
            double averDistance= Statistic.average(distance);

            for (int j = 0; j < this.dimensionSize; j++) {
                cvalue = this.population.get(i).getVariableValue(j);
                if (this.randGenerator.nextDouble() > this.pa||distance[j]>averDistance) {
    ...
    public static void main(String[] args) {
        ...
        CS_variants cs = new CS_variants(problem, 30*2*5000, 30, 0.25, 1.5);
        cs.run();
